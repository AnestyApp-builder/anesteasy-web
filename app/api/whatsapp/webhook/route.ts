...
import { validateMetaSignature, sendWhatsAppMessage } from '@/lib/providers/whatsapp/meta';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 3. Lógica de Mensagem de Texto (Vinculação, Confirmação e Secretária)
    if (message.type === 'text') {
      ...
      const { data: extraction } = await supabaseAdmin
        ...
        .in('status', ['awaiting_name', 'awaiting_anesthesia', 'awaiting_procedure', 'awaiting_secretary', 'awaiting_confirmation'])
        ...
        if (extraction) {
          ...
          if (extraction.status === 'awaiting_anesthesia') {
            ...
            if (procSugerido && procSugerido !== 'Não identificado') {
              await sendWhatsAppMessage(from, `📝 Identifiquei o procedimento: *${procSugerido}*.\n\nConfirma ou deseja alterar? (Digite *SIM* ou o *PROCEDIMENTO CORRETO*)`);
            } else {
          ...
          if (extraction.status === 'awaiting_procedure') {
            ...
            await sendWhatsAppMessage(from, "🤝 *Deseja disponibilizar este procedimento no link seguro da secretária?*\n\n1 - Sim\n2 - Não");
            return NextResponse.json({ status: 'procedure_confirmed' });
          }
...
async function handleSecretarySelection(from: string, text: string, extraction: any) {
  ...
  if (proc) {
    ...
    const msgFinal = showToSecretary 
      ? `✅ *Sucesso!* Procedimento registrado e visível para a secretária.`
      : `✅ *Sucesso!* Procedimento registrado (privado).`;
    await sendWhatsAppMessage(from, msgFinal);
  }
}
